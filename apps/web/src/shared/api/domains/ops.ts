...
export function getOpsApiUsageSummary(
  token: string,
  window: OpsWindow,
): Promise<OpsSummaryResponse> {
  return apiRequest<OpsSummaryResponse>(
    `/ops/api-usage/summary?window=${encodeURIComponent(window)}`,
    { token },
  );
}

export function getOpsApiUsageTimeseries(
  token: string,
  window: OpsTimeseriesWindow,
): Promise<OpsTimeseriesResponse> {
  return apiRequest<OpsTimeseriesResponse>(
    `/ops/api-usage/timeseries?window=${encodeURIComponent(window)}&bucket=hour`,
    { token },
  );
}

export function getOpsIngestHealth(
  token: string,
  eventLimit: number = 20,
): Promise<OpsIngestHealthResponse> {
  return apiRequest<OpsIngestHealthResponse>(
    `/ops/db/ingest-health?event_limit=${encodeURIComponent(String(eventLimit))}`,
    {
      token,
    },
  );
