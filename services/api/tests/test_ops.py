...
def test_ops_routes_return_data_for_admin(client, monkeypatch):
    ...
    assert ingest_health.json()["active_leagues"] == ["NBA", "WNBA", "MLB", "MLS", "WORLD_CUP"]
    ...
    assert summary_json["runtime"]["active_leagues"] == ["NBA", "WNBA", "MLB", "MLS", "WORLD_CUP"]
    assert len(summary_json["runtime"]["league_settings"]) == 5
