...
def test_canonical_alert_settings():
    assert default_alert_settings("basketball", "close_game_late") == AlertSettings(
        is_enabled=True,
...
def test_unsupported_alert_type_is_rejected():
    with pytest.raises(ValueError, match="Unsupported alert type"):
        default_alert_settings("basketball", "score_changed")
...
def test_preference_and_game_override_precedence():
    ...
    assert resolve_alert_settings("basketball", "close_game_late", preference, game_override) == AlertSettings(
        is_enabled=True,
...
def test_sparse_overrides_store_only_values_different_from_baseline():
    ...
    assert (
        apply_sparse_overrides(
            preference, settings, default_alert_settings("basketball", "close_game_late")
        )
        is True
    )
...
def test_sparse_overrides_remove_values_equal_to_sport_settings():
    game_override = UserGameAlertOverride(
        user_id=1, game_id=1, alert_type="close_game_late"
    )
    sport_settings = AlertSettings(
        is_enabled=False,
        close_game_margin_threshold=3,
        close_game_time_threshold_seconds=90,
    )
    ...
    assert apply_sparse_overrides(game_override, game_settings, sport_settings) is True
