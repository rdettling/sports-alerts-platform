...
def list_games(
    ...
    stmt = select(Game).where(Game.competition.in_(active_competitions)).where(Game.scheduled_start_time >= lower, Game.scheduled_start_time <= upper).order_by(Game.scheduled_start_time.asc()).limit(limit)
    if competition:
    ...
    odds_rows = db.scalars(select(GameOddsCurrent).options(selectinload(GameOddsCurrent.outcomes)).where(GameOddsCurrent.game_id.in_(game_ids))).all()
